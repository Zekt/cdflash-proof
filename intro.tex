...
We will first define an abstract (and simpler) transition system~\Spec\ that acts as a specification of the actual program transition system~\Prog.
...
\todo{crash-deterministic fragment?}\
...
The Agda proof%
\todo{change to ``The majority of the Agda proof'' if eventually we need to prove stuff about the exact definitions of the simulation relations, the invariants, etc}\
...
Next we will formally define the simulation relation between \Prog~and~\Spec, but instead of considering the entire~\Prog, it suffices to focus on a sub-system~\ProgInv\ of~\Prog\ which considers only the states satisfying the invariants and the transitions that ``respect'' the invariants.
In \Cref{sec:ProgInv} we will define~\ProgInv\ and prove some of its properties.
The simulation relation between \ProgInv~and~\Spec, to be defined in \cref{sec:sim}, will be easier to handle (compared to the one between \Prog~and~\Spec), and we will still be able to prove crash-determinacy since the property is only about well-formed fragments,%
...
The proof of crash-determinacy will then be presented in \cref{sec:proof}.
...
\end{theorem}
\begin{proof}
	From $\mathit{initialisation}$ and repeatedly applying \cref{lemma-1} to the first relation, we have $\mathit{AR(s, t)}$ and $\mathit{AR(s', t')}$.
